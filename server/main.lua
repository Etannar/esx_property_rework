RegisterServerEvent('esx_property_garage:debug')
RegisterServerEvent('esx_property_garage:modifystate')
RegisterServerEvent('esx_property_garage:pay')
RegisterServerEvent('esx_property_garage:payhealth')
RegisterServerEvent('esx_property_garage:logging')
...
RegisterServerEvent('esx_property:getOwnedProperties')
AddEventHandler('esx_property:getOwnedProperties', function(id)
	local _source = source
	local xPlayer  = ESX.GetPlayerFromId(_source)
	local owner = xPlayer.getIdentifier()
	local properties = {}
	MySQL.Async.fetchAll('SELECT * FROM owned_properties WHERE owner = @owner', {['@owner'] = owner}, function(result)
		for i=1, #result, 1 do
			...
			if result[i].name ~= nil then
				sentProperty(result[i].name, _source)
			end
		end
...
function sentProperty(name, _source)
	TriggerClientEvent('esx_property:setPropertyOwned', _source, name, true)
end

RegisterServerEvent('esx_property:getServerOwnedProperties')
AddEventHandler('esx_property:getServerOwnedProperties', function(id)
	local _source = source
	local properties = {}
	MySQL.Async.fetchAll('SELECT * FROM owned_properties', {}, function(result)
		for i=1, #result, 1 do
			...
			if result[i].name ~= nil then
				sentServerProperty(result[i].name, _source)
			end
		end
	end)
end)

function sentServerProperty(name, _source)
	TriggerClientEvent('esx_property:setServerPropertyOwned', _source, name, true)
end
...
ESX.RegisterServerCallback('esx_property_garage:getVehicles', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local vehicules = {}

	MySQL.Async.fetchAll("SELECT * FROM private_vehicles WHERE owner=@identifier",{['@identifier'] = xPlayer.getIdentifier()}, function(data) 
...
ESX.RegisterServerCallback('esx_property_garage:stockv',function(source,cb, vehicleProps)
	local isFound = false
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	...
	local plate = vehicleProps.plate
		...
		for _,v in pairs(vehicules) do
			if(plate == plate)then
				...
				MySQL.Sync.execute("UPDATE private_vehicles SET vehicle=@vehprop WHERE plate=@plate",{['@vehprop'] = vehprop, ['@plate'] = plate})
				isFound = true
				break
			end		
		end
...
AddEventHandler('esx_property_garage:modifystate', function(plate, state)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local vehicules = getPlayerVehicles(xPlayer.getIdentifier())
	local state = state
	...
	for _,v in pairs(vehicules) do
		MySQL.Sync.execute("UPDATE private_vehicles SET state =@state WHERE plate=@plate",{['@state'] = state , ['@plate'] = plate})
		break		
	end
end)	

ESX.RegisterServerCallback('esx_property_garage:getOutVehicles',function(source, cb)	
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local vehicules = {}

	MySQL.Async.fetchAll("SELECT * FROM private_vehicles WHERE owner=@identifier AND state=false",{['@identifier'] = xPlayer.getIdentifier()}, function(data) 
		for _,v in pairs(data) do
			local vehicle = json.decode(v.vehicle)
...
ESX.RegisterServerCallback('esx_property_garage:checkMoney', function(source, cb)
...
AddEventHandler('esx_property_garage:pay', function()
...
function getPlayerVehicles(identifier)
	...
	local vehicles = {}
	local data = MySQL.Sync.fetchAll("SELECT * FROM private_vehicles WHERE owner=@identifier",{['@identifier'] = identifier})	
	for _,v in pairs(data) do
		local vehicle = json.decode(v.vehicle)
...
AddEventHandler('onMySQLReady', function()
	...
	MySQL.Sync.execute("UPDATE private_vehicles SET state=true WHERE state=false", {})

end)

AddEventHandler('esx_property_garage:payhealth', function(price)
	local xPlayer = ESX.GetPlayerFromId(source)
